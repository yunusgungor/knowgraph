...
async def ingest_conversation_directory(
    ...
    ----
        ...
        output_dir: Directory to save markdown files (if None, uses temp dir)
        include_patterns: File patterns to include (e.g., ["*.aichat", "*.json"])

    Returns:
    -------
        List of (markdown_content, output_path) tuples

    """
    if include_patterns is None:
        include_patterns = ["*.aichat", "*.json", "*.txt"]

    results = []

    for pattern in include_patterns:
        for conv_file in conversations_dir.glob(f"**/{pattern}"):
            try:
            ...
            except ConversationIngestorError:
