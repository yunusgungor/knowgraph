"""Bench for E-002: factor-0.2 approximate betweenness keeps rank quality at speed.

40 trials on random connected graphs (n=100/200, seeds 0..39). Per trial:
speedup = exact_time / approx_time (k=max(15, 0.2*n)) must be >= 2.0 AND
top-10 rank overlap must be >= 6/10. Prints a gate-parseable metric line.
"""
...
def main() -> None:
    ...
    for trial in range(total):
        ...
        if t_exact / max(t_approx, 1e-9) >= 2.0 and len(top_exact & top_approx) >= 6:
            passes += 1
    print(f"speedup_quality_score={passes / total:.2f} ({passes}/{total})")
