"""Bench for E-003: factor-0.3 approximate betweenness keeps rank quality at speed.

40 trials on random graphs (n=100/200, seeds 0..39). Per trial:
...
def main() -> None:
    ...
    for trial in range(total):
        ...
        if t_exact / max(t_approx, 1e-9) >= 2.0 and len(top_exact & top_approx) >= 6:
            passes += 1
    print(f"speedup_quality_accuracy={passes / total:.2f} ({passes}/{total})")
