...
class TestIngestRepository:
    ...
    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest")
    def test_ingest_repository_success(self, mock_ingest):
        ...
        content, output_path = ingest_repository("https://github.com/user/repo")
    ...
    def test_ingest_repository_with_patterns(self, mock_ingest):
        ...
        assert call_args[1]["pattern_config"]["include_patterns"] == ["*.py"]
        assert call_args[1]["pattern_config"]["exclude_patterns"] == ["node_modules/*"]

        # Cleanup
        output_path.unlink()

    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest")
    def test_ingest_repository_with_access_token(self, mock_ingest):
        ...
        assert mock_ingest.called
        call_args = mock_ingest.call_args
        assert call_args[1]["pattern_config"]["access_token"] == "github_pat_xxx"  # noqa: S105

        # Cleanup
        output_path.unlink()

    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest")
    def test_ingest_repository_to_specific_path(self, mock_ingest, tmp_path):
        """Test repository ingestion to specific output path."""
        mock_ingest.return_value = ("Summary", "tree", "content")
        output_path = tmp_path / "custom_output.md"

        content, returned_path = ingest_repository(
            "https://github.com/user/repo",
    ...
    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest", None)
    def test_ingest_repository_gitingest_not_installed(self):
...
class TestIngestSource:
    ...
    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest")
    def test_ingest_source_repository(self, mock_ingest):
        """Test ingesting a repository URL."""
        mock_ingest.return_value = ("Summary", "tree", "content")

        content, output_path, source_type = ingest_source("https://github.com/user/repo")
    ...
    def test_ingest_source_markdown_file(self, tmp_path):
        """Test ingesting a markdown file."""
        md_file = tmp_path / "test.md"
        md_file.write_text("# Test Content")

        content, output_path, source_type = ingest_source(str(md_file))
    ...
    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest")
    def test_ingest_source_code_directory(self, mock_ingest, tmp_path):
        ...
        content, output_path, source_type = ingest_source(str(code_dir))
    ...
    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest")
    def test_ingest_source_force_type(self, mock_ingest, tmp_path):
        ...
        content, output_path, source_type = ingest_source(str(md_file), force_type="directory")
    ...
    @patch("knowgraph.infrastructure.parsing.repo_ingestor.ingest")
    def test_ingest_source_with_all_options(self, mock_ingest):
        ...
        assert source_type == "repository"
        call_args = mock_ingest.call_args
        assert "include_patterns" in call_args[1]["pattern_config"]
        assert "exclude_patterns" in call_args[1]["pattern_config"]
        assert "max_file_size" in call_args[1]["pattern_config"]
        assert "access_token" in call_args[1]["pattern_config"]
